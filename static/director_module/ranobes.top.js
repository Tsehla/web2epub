...
function toc_contents_scraping(){
    ...
    //var  toc_firt_page_link = "https://ranobes.top/up/earths-greatest-magus/page/7/".split("/");
    ...
    if(book_main_age && book_main_age[0] && book_main_age[0].href){ //https://ranobes.top/novels/693420-supremacy-games-v216510.html

        chapter_links_container.book_cover_image_link  = book_main_age[0].href;//save book image url

       rectrive_book_toc_pages(
        document.getElementById("request_html_container").querySelectorAll(".r-fullstory-chapters-foot a")[1].href.replace(b, 'https://ranobes.top'), true
       );//redirect and request toc contents webpage 

       // console.log( b,document.getElementById("request_html_container").querySelectorAll(".r-fullstory-chapters-foot a")[1].href.replace(b, 'https://ranobes.top'))
       return;
    }

    if(!book_main_age || !book_main_age[0] || !book_main_age[0].href){//give some frinedly advice//https://ranobes.top/novels/693420-supremacy-games-v216510.html

        toc_container_pages = document.getElementById("request_html_container").querySelectorAll(".pages a");

        alert_box_1(`"if problem quck solution : give link with pages/chapter like [https://ranobes.top/chapters/304010/page/20/]"Provide link with ".html" at end to extrach book Image i.e [ https://ranobes.top/novels/693420-supremacy-games-v216510.html ], instead [https://ranobes.top/up/earths-greatest-magus/] for the book.`, "","", "alert");//give alert
      ...
      links_dom_array.forEach(function(data, index) {
        ...
        //    console.log(' ::::: ',data.querySelector("div h6.title").textContent, ' ::::: ',data.textContent.trim().split(":")[0],' :::: ',data.innerHTML)
                chapter_links_container.book_chapters.unshift({
                    chapter_link: data.href.trim().replace(b, 'https://ranobes.top'),//replace link referal to local server to remote server hosting the book
       ...
       //    console.log(toc_container_pages.length, retrived_toc_pages);
        ...
        // if(retrived_toc_pages != 2){ 
            ...
            if( retrived_toc_pages == 1){//first time function run
                ...
                toc_firt_page_link_template = toc_firt_page_link_template.trim().replace(b, 'https://ranobes.top')
